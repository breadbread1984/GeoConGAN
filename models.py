...
def RegNet(input_shape = (256,256,3), heatmap_size = (32,32), coeff = 1.):
    ...
    lambda x, shape: 
      tf.tile(
        tf.reshape(
          tf.stack(
            [
              tf.tile(tf.reshape(tf.range(tf.cast(shape[1], dtype = tf.float32), dtype = tf.float32), (1, shape[1])), (shape[0], 1)), # every row is composed of different x
              tf.tile(tf.reshape(tf.range(tf.cast(shape[0], dtype = tf.float32), dtype = tf.float32), (shape[0], 1)), (1, shape[1]))  # every column is composed of different y
  ...
  results = tf.keras.layers.Lambda(lambda x, shape: tf.tile(tf.reshape(x,(-1, 21, 1, 2)), (1, 1, shape[0] * shape[1], 1)), arguments = {'shape': heatmap_size})(results); # results.shape = (batch, 21, heatmap.h * heatmap.w, 2)
  results = tf.keras.layers.Lambda(lambda x: x[0] - x[1])([results, grid]);
  results = tf.keras.layers.Lambda(lambda x, c: (tf.math.square(x[...,0]) + tf.math.square(x[...,1])) / c, arguments = {'c': coeff})(results); # distance from joints to all positions
  results = tf.keras.layers.Lambda(lambda x, c, pi: tf.math.exp(-x / 2.) / (2. * pi * c), arguments = {'c': coeff, 'pi': np.pi})(results); # heatmap of 21 joints
